...
def create_df(date, warehouse, capacity):
    ...
    groupedby = sub_orders[["delivery_location", "order_total_volume", "order_id", "n_units"]].groupby("delivery_location").agg({"order_total_volume": sum, "n_units": sum, "order_id": list}).reset_index()
    ...
    over_capacity_orders = sub_orders[["delivery_location", "order_total_volume", "order_id", "n_units"]][sub_orders["delivery_location"].isin(over_capacity_cities)]
    over_capacity_orders["order_id"] = over_capacity_orders["order_id"].apply(lambda order_id:[order_id])
    ...
    if warehouse in groupedby.delivery_location.tolist():
        ...
        house = pd.DataFrame(dict(delivery_location=[warehouse], order_total_volume=[0], n_units=[0], order_id=[None]))
        groupedby = pd.concat([house, groupedby], axis=0)
        
    return pd.merge(groupedby, cities[["city", "lat", "lng"]], left_on="delivery_location", right_on="city")
...
def _distance_calculator(_df):
    ...
    for i in range(len(_df)):
        for j in range(len(_df)):
            ...
            _distance_result[i][j] = haversine.haversine((_df.iloc[i]["lat"],_df.iloc[i]["lng"]), 
...
def save_solution(data, manager, routing, solution):
    ...
    for vehicle_id in range(data['num_vehicles']):
        ...
        vehicules.append(dict(route_distance=route_distance / 1000, stops_vehicle=stops_vehicle,
...
def make_routes(output, df, date, warehouse):
  ...
  for route in output:
    if route["route_load"] > 0:
      ...
      route_row["n_units"] = np.sum([df["n_units"][k] for k in route["stops_vehicle"]])
      orders_list = [" > ".join(df["order_id"][k]) for k in route["stops_vehicle"] if df["order_id"][k]]
      route_row["orders"] = " > ".join([order for order in orders_list if order != ""])
      route_row["from_warehouse"] = warehouse
      route_row["route_date"] = date[:10]
      route_row["stops"] = " > ".join([df["city"][k] for k in route["stops_vehicle"]])
